...
import {
  applyNoticeHomeAction,
  applyNoticePinAction,
...
export const NOTICE_HOME_MAX = 5;
...
/** 공지사항 관리 목록 — 나무말미 공지 / WCO 보도자료와 동일 패턴 */
export function NoticeListClient({ items, loadError, registerHref }: Props) {
  ...
  function toggleHome(id: string) {
    setHomeIds((prev) => {
      ...
      if (next.has(id)) {
        ...
        if (next.size >= NOTICE_HOME_MAX) {
          setFeedback({
            tone: "error",
            message: `메인 노출은 최대 ${NOTICE_HOME_MAX}건까지 선택할 수 있습니다.`,
          });
          return prev;
        }
        next.add(id);
  ...
  function applyHome() {
    if (
      !window.confirm(
        `메인 노출 ${homeIds.size}건을 반영하시겠습니까? (최대 ${NOTICE_HOME_MAX}건)`,
      )
    ) {
      return;
    }
    ...
    startTransition(async () => {
      const result = await applyNoticeHomeAction(fd);
      if (result.error) {
  ...
  function applyPin() {
    ...
    startTransition(async () => {
      const result = await applyNoticePinAction(fd);
      if (result.error) {
        setFeedback({ tone: "error", message: result.error });
        return;
      }
    ...
    <div>
      ...
      >
        <p className="border-b border-black/10 px-4 py-2.5 text-sm text-[#6B6B6B]">
          [공지사항] 메인 노출 {homeCount}/{NOTICE_HOME_MAX} · 상단 고정{" "}
          {pinCount}건
        </p>
        <div className="overflow-x-auto">
          ...
          >
            <caption className="sr-only">
              공지사항 목록. 총 {paged.total}건. 페이지당 {ADMIN_LIST_PAGE_SIZE}
              건.
            </caption>
            <colgroup>
            ...
            <tbody>
              {paged.items.length === 0 ? (
                <AdminEmptyTableRow
                  colSpan={7}
                  message={adminEmptyListMessage("공지사항", hasActiveFilter)}
                />
              ) : (
