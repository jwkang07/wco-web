...
const env = loadEnv();
const sql = readFileSync("supabase/alter-activity-body-html.sql", "utf8");
...
try {
  ...
  const { rows } = await client.query(`
    ...
    where table_schema='public'
      and table_name in ('performances','press_articles')
      and column_name='body_html'
    order by table_name
  `);
  console.log("OK body_html:", rows.map((r) => r.table_name).join(", "));
} finally {
  await client.end();
}
