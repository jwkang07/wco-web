...
const env = loadEnv();
const sql = readFileSync("supabase/alter-press-pin-views.sql", "utf8");
...
try {
  ...
  const { rows } = await client.query(`
    ...
    where table_schema = 'public'
      ...
      and column_name in ('is_pinned', 'view_count', 'show_on_home')
    order by column_name
  `);
  console.log("OK press_articles pin/views", rows);
} finally {
  await client.end();
}
